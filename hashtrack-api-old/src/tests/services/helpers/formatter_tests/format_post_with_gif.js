...
export default describe('A Tweet With a GIFs', () => {
  const tweetWithGIF = TweetFactory.build({}, { tweet_type: TWEET_TYPES.GIF });
  const formattedPost = formatPost(tweetWithGIF);

  describe('formatted post object', () => {
    it('should have the same ID as the original object', () => {
      const { twitter_post_id } = formattedPost;
      expect(twitter_post_id).toEqual(tweetWithGIF.id_str);
    })

    it('should have the same created_at time as the original object', () => {
      const { created_at } = formattedPost;
      expect(created_at).toEqual(tweetWithGIF.created_at);
    });

    it('should have a deconstructed date based on the original objects created_at value', () => {
      const { deconstructed_date } = formattedPost;
      expect(deconstructed_date).toEqual(createDeconstructedDate(tweetWithGIF.created_at));
    });

    it('should have a source derived from the users screen_name', () => {
      const { source } = formattedPost;
      expect(source).toEqual(createSourceLink(tweetWithGIF.user.screen_name, tweetWithGIF.id));
    });
    ...
    describe('media', () => {
      ...
      it('should only have one media object', () => {
        ...
        expect(tweetWithGIF.entities.media.length).toBe(1)
        expect(tweetWithGIF.extended_entities.media.length).toBe(1)
      })

      it(`with a type of ${media_types.GIF}`, () => {
        const [mediaObj] = media
        const [originalMediaObj] = tweetWithGIF.entities.media;
        const [extendedMediaObj] = tweetWithGIF.extended_entities.media;
      ...
      it('should have the same length as the original object', () => {
        expect(media.length).toEqual(tweetWithGIF.entities.media.length);
      });
      ...
      it('should contain objects derived from the original objects media extended_entities', () => {
        tweetWithGIF.entities.media.forEach(mediaObj => {
          expect(
        ...
        tweetWithGIF.extended_entities.media.forEach(extendedMediaObj => {
          expect(
    ...
    describe('hashtags', () => {
      ...
      it('should have the same length as the original object', () => {
        expect(hashtags.length).toEqual(tweetWithGIF.entities.hashtags.length);
      });

      it('should be derived from the original objects hashtags entities', () => {
        tweetWithGIF.entities.hashtags.forEach(hashtag => {
